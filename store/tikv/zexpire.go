...
func (tikv *Tikv) ZTTL(txn interface{}, key []byte) (ttl int64, err error) {
	ttl, err = tikv.ZPTTL(txn, key)
	if ttl > 0 {
		ttl = ttl / 1000
	}
	return
...
func (tikv *Tikv) ZPTTL(txn interface{}, key []byte) (ttl int64, err error) {
	var (
		dataKey  []byte
		ttlKey   []byte
		value    []byte
	...
	dataKey = utils.EncodeZSetKey(key)
...
func (tikv *Tikv) ZPExipre(txn interface{}, key []byte, ts int64) (ret int, err error) {
	var (
		v         []byte
		ttlKey    []byte
		ttlValue  []byte
	...
	v, err = tikv.Get(txn, utils.EncodeZSetKey(key))
	if err != nil {
		return
	}
	if v == nil {
	...
	if ttlValue != nil {
		...
		//old expire key
		expireKey = utils.EncodeExpireKey(key, utils.ZSET_TYPE, int64(ttlOld))
	...
	//set expire key
	expireKey = utils.EncodeExpireKey(key, utils.ZSET_TYPE, ts)
